...
function PracticeDetailCard({ activePractice, refLabel }) {
  return (
    <AnimatePresence mode="wait">
      ...
      >
        {/* Dynamic Background Image & Gradient Scrim */}
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-surface-light/95 via-surface-light/85 to-surface-light/75 dark:from-[#0F0F12]/95 dark:via-[#0F0F12]/85 dark:to-[#0F0F12]/75 backdrop-blur-[2px] transition-colors duration-300" />
        </div>

        {/* Content Layer */}
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-6 pb-4 border-b border-theme">
            <span className="font-mono text-xs font-bold tracking-[0.2em] uppercase text-primary-light dark:text-primary-dark">
              {refLabel}
            </span>
          ...
          <div className="pt-2">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {activePractice.keyServices?.map((service, idx) => (
                ...
                >
                  <Check className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0 opacity-90" />
                  <span>{service}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="mt-10 pt-6 border-t border-theme flex flex-col sm:flex-row sm:items-center justify-between gap-5 relative z-10">
          <Link
            to={`/schedule?service=${activePractice.id}`}
            className="inline-flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-xl bg-slate-950 text-white dark:bg-white dark:text-slate-950 font-mono text-xs font-bold tracking-widest uppercase hover:bg-black dark:hover:bg-slate-100 transition-all duration-300 shadow-lg group/btn active:scale-98"
          >
          ...
          <div className="flex items-center justify-between sm:justify-end gap-4 font-mono text-[10px] tracking-widest uppercase text-secondary-light dark:text-secondary-dark">
            <span>STATUTORY AUDIT DESK</span>
            <span>•</span>
            <span>WB / ALL INDIA</span>
          </div>
        </div>
      </motion.div>
    </AnimatePresence>
